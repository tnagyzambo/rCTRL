...
const int sensorJsonCapacity =
    JSON_OBJECT_SIZE(12); // right now we have, LC, tankps1, tankps2, ccPS0, ccPS1, thermo 1, 2, 3
...
void setup() {
    ...
    if (LoadCell.begin(0x40) == false) // Connect to the Load Cell using the defaults: Address 0x40 is the board without
                                       // the direct connect connector. Defualt Wire port
    {
    ...
    if (TankPS.begin(0x44) == false) // Connect to the Pressure sensor using the defaults: Address 0x44 is the board
                                     // with the direct connector. Defaul tWire port
    {
        Serial.println(
            F("Error I2C bus to the Pressure Sensor. Check address and I2C lines. Please check wiring. Freezing."));
        while (1)
            ;
    }
    ...
    LoadCell.setLCSlopeAndOffset(
        1, 0);        // 1, 0 is slope of 1 and offset of zero. Basically returning the differential voltage vin.
...
void loop() {
    ...
    if (CURRENT_TIME - HS_PREVIOUS >= HS_PERIOD) {
        // readPS() and readLC have a small delay in it incase DRDY is not ready. It won't go longer then
        // ADS122C04_CONVERSION_TIMEOUT It should never be delayed as the mode is set to continuous

        // Read LC value
        lc_value =
            LoadCell.readLC(); // Use the read function. Units depend on calibration. The output voltage true voltage (aka raw/gain)
