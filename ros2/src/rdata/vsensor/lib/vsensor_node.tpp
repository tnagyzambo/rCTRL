template <typename T>
Node<T>::Node(const char *nodeName, std::chrono::milliseconds period) : rclcpp_lifecycle::LifecycleNode(nodeName) {
    this->nodeName = nodeName;
    this->loggerTopicName = createLoggerTopicName();
    this->period = period;

    // Construct timer and immediately stop it
    this->timer = this->create_wall_timer(period, std::bind(&Node<T>::timerCallback, this));
    this->timer->cancel();
}
...
template <typename T>
rclcpp_lifecycle::node_interfaces::LifecycleNodeInterface::CallbackReturn Node<T>::on_configure(
...
// If this returns ERROR the node run the errorProcessing callback
template <typename T>
rclcpp_lifecycle::node_interfaces::LifecycleNodeInterface::CallbackReturn Node<T>::on_activate(
    const rclcpp_lifecycle::State &) {
...
// If this returns ERROR the node run the errorProcessing callback
template <typename T>
rclcpp_lifecycle::node_interfaces::LifecycleNodeInterface::CallbackReturn Node<T>::on_deactivate(
    const rclcpp_lifecycle::State &) {
...
template <typename T>
rclcpp_lifecycle::node_interfaces::LifecycleNodeInterface::CallbackReturn Node<T>::on_cleanup(
    const rclcpp_lifecycle::State &) {
...
template <typename T>
rclcpp_lifecycle::node_interfaces::LifecycleNodeInterface::CallbackReturn Node<T>::on_shutdown(
    const rclcpp_lifecycle::State &) {
...
template <typename T> uint Node<T>::calcElapsedTime() {
...
template <typename T> void Node<T>::timerCallback() {}

template <typename T> std::string Node<T>::createLoggerTopicName() {
