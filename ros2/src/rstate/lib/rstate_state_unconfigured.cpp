...
namespace rstate
{
    ...
    void Unconfigured::enter(Node *node)
    {
        RCLCPP_INFO(node->get_logger(), "Network is unconfigured");
    }

    rclcpp_action::GoalResponse Unconfigured::handleGoal(
        Node *node,
    ...
    void Unconfigured::handleAccepted(
        ...
        const std::shared_ptr<rclcpp_action::ServerGoalHandle<action::Transition>> goalHandle)
    {
        std::thread{std::bind(Unconfigured::onTransition, this, std::placeholders::_1, std::placeholders::_2), node, goalHandle}.detach();
    }
